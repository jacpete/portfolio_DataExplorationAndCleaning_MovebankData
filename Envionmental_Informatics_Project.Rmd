...
#  transformation
sfFullEurope <- sfFull %>% st_transform(crs = 102016) %>%
  jpfxns2::sf_XYtoCols(.)
...
test plot 1 ref with Fig. \@ref(fig:Plot-Individuals-Per-Study-Region) and hopefully it works
...
First,  I identified the distribution of individuals across the study regions (\hyperlink{Plot-Individuals-Per-Study-Region}{{\color{blue}{\underline{`r figNum("Plot-Individuals-Per-Study-Region", display = "cite")`}}}}).
I then mapped the points to see where exactly the birds from the different study regions were located and look for spatial outliers (\hyperlink{Plot-Spatial-Map}{{\color{blue}{\underline{`r figNum("Plot-Spatial-Map", display = "cite")`}}}}). Next I explored the distribution of movement records among individuals and study regions (\hyperlink{Plot-Individual-Point-Distribution}{{\color{blue}{\underline{`r figNum("Plot-Individual-Point-Distribution", display = "cite")`}}}}). This would help identify individuals that had a lot less relocations as well as the variation in the number of relocations gathered between study regions.
I also plotted the distribution of the amount of time between consecutive points for each individual (\hyperlink{Plot-Time-Between-Points-Indiv}{{\color{blue}{\underline{`r figNum("Plot-Time-Between-Points-Indiv", display = "cite")`}}}}). Large gaps in time between successive points would make the data less useful and such gaps should be removed in further analyses.
...
To check for this, I used boxplots to plot the standardized distance over time by each month in the entire time series (\hyperlink{Plot-Dist-Traveled-Over-Time}{{\color{blue}{\underline{`r figNum("Plot-Dist-Traveled-Over-Time", display = "cite")`}}}}).
...
The patterns identifying the distribution of points across time and study regions were summarized best in the final two graphs that plot the count of points occurring in the years of the study and across individual months (Figs. \hyperlink{Count-of-points-per-year}{{\color{blue}{\underline{`r figNum("Count-of-points-per-year", display = "num")`}}}} and \hyperlink{Count-of-points-per-month}{{\color{blue}{\underline{`r figNum("Count-of-points-per-month", display = "num")`}}}} respectively).
Because there is a noticeable lack of points recorded during the winter months (\hyperlink{Count-of-points-per-month}{{\color{blue}{\underline{`r figNum("Count-of-points-per-month", display = "cite")`}}}}), I will only be estimating home ranges in the summer breeding months.
...
\newpage
\hypertarget{Plot-Individuals-Per-Study-Region}{{\color{blue}{}}}
...
jpfxns2::adaptiveCenterCaption(chunckName="Plot-Individuals-Per-Study-Region",
                               caption = "Distribution of individuals among \
                               study regions", capFxn = "figNum")
```

\newpage
\hypertarget{Plot-Spatial-Map}{{\color{blue}{}}}
```{r Plot-Spatial-Map, warning=FALSE, message=FALSE, fig.align='center', cache=TRUE}
...
```{r, echo=FALSE, results="asis"}
jpfxns2::adaptiveCenterCaption(chunckName="Plot-Spatial-Map",
                               caption = "Distribution of movement data across \
                               all study regions", capFxn = "figNum")
```

\newpage
\hypertarget{Plot-Individual-Point-Distribution}{{\color{blue}{}}}
```{r Plot-Individual-Point-Distribution, warning=F, fig.align='center'}
...
```{r, echo=FALSE, results="asis"}
jpfxns2::adaptiveCenterCaption(chunckName="Plot-Individual-Point-Distribution",
                               caption = "Distribution of movement relocations \
                               among individuals and study regions",
                               capFxn = "figNum")
```

\newpage
\hypertarget{Plot-Time-Between-Points-Indiv}{{\color{blue}{}}}
```{r Plot-Time-Between-Points-Indiv, warning=FALSE, fig.height=6, fig.align='center'}
...
```{r, echo=FALSE, results="asis"}
jpfxns2::adaptiveCenterCaption(chunckName="Plot-Time-Between-Points-Indiv",
                               caption = "Distribution of the length of time \
                               between successive relocations on bird data by \
                               individuals and study regions",
                               capFxn = "figNum")
```

\newpage
\hypertarget{Plot-Dist-Traveled-Over-Time}{{\color{blue}{}}}
```{r Plot-Dist-Traveled-Over-Time, warning=FALSE, fig.height=6, fig.align='center'}
...
```{r, echo=FALSE, results="asis"}
jpfxns2::adaptiveCenterCaption(chunckName="Plot-Dist-Traveled-Over-Time",
                               caption = "Distribution of distance traveled, \
                               standardized by an hour, during each movement \
                               across time", capFxn = "figNum")
```

\newpage
\hypertarget{Count-of-points-per-year}{{\color{blue}{}}}
```{r Count-of-points-per-year, echo=FALSE, warning=F, message=FALSE, fig.align='center'}
...
```{r, echo=FALSE, results="asis"}
jpfxns2::adaptiveCenterCaption(chunckName="Count-of-points-per-year",
                               caption = "Count of points occurring in the \
                               different years of the studies",
                               capFxn = "figNum")
```

\newpage
\hypertarget{Count-of-points-per-month}{{\color{blue}{}}}
```{r Count-of-points-per-month, echo=FALSE, warning=F, message=FALSE, fig.align='center'}
...
```{r, echo=FALSE, results="asis"}
jpfxns2::adaptiveCenterCaption(chunckName="Count-of-points-per-month",
                               caption = "Count of points occurring in the \
                               different months of the year", capFxn = "figNum")
```
...
sfQualityCheck <- sfFullEurope %>% st_transform(crs = 102016) %>%
...
\hypertarget{Draw-Summary-Table}{{\color{blue}{}}}
...
```{r results="asis"}
jpfxns2::adaptiveCenterCaption(chunckName="Draw-Summary-Table",
                               caption = "Table summarizing the results of the \
                               data quality checking", capFxn = "TabNum")
```
...
dispDF <- sfGood %>% st_transform(crs = 102016) %>% #reproject to equidistant
...
For home range analysis, I only focused on the summer breeding season as there was a lack of data during the winter months  (\hyperlink{Count-of-points-per-month}{{\color{blue}{\underline{`r figNum("Count-of-points-per-month", display = "cite")`}}}}).
...
When examining the regional differences between winter and breeding sites using max displacement distance, I found that the only significant regional difference was between the Greenland and Barents Sea populations (p = `r round(dispTukeyPvalues[1], 2)`; \hyperlink{Plot-Displacement-Distance}{{\color{blue}{\underline{`r figNum("Plot-Displacement-Distance", display = "cite")`}}}}).
...
The ANOVA found no significant variation between regions (p > 0.05; \hyperlink{Plot-Home-Range-Size}{{\color{blue}{\underline{`r figNum("Plot-Home-Range-Size", display = "cite")`}}}}).
...
```{r Plot-Displacement-Distance, echo=FALSE, warning=F, fig.align='center'}
...
```{r results="asis"}
jpfxns2::adaptiveCenterCaption(chunckName="Plot-Displacement-Distance",
                               caption = "Distribution of maximum displacement \
                               distance (km) in barnacle geese between winter \
                               and summer ranges across breeding \
                               sub-populations", capFxn = "figNum")
```


```{r Plot-Home-Range-Size, echo=FALSE, warning=F, fig.align='center'}
...
```{r results="asis"}
jpfxns2::adaptiveCenterCaption(chunckName="Plot-Home-Range-Size",
                               caption = "Distribution of summer home range \
                               size ($km^2$) in barnacle geese across \
                               breeding sub-populations", capFxn = "figNum")
```
